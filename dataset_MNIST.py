...
class MNIST(datasets.MNIST):
    def __init__(self, padding=9, didactic=True, distractor=True, unique_didactic=False, false_didactic=False,
                 fixed_didactic=True, random_labels=False, **kwargs):
        """

        :param padding: # of pixels to padd the image, also the width/height of the didactic & distractor signals
        :param didactic: Adds a didactic signal to the original image
        :param distractor: When true adds a distractor signal which does not overlap with the didactic signal
        :param unique_didactic: Each label has its own didactic signal
        :param false_didactic: Adds a panda with s&p distractor in other didactic position
        ...
        # Read in distractor picture and pre-compute possible distractor coordinates if the didactic
        # signal is in a fixed location (fixed_didactic=True)
        if distractor:
            distractor = imread('/home/eric/PycharmProjects/XAI/poop-emoji.png')
            distractor_resized = resize(distractor, (self.padding, self.padding))
            distractor_resized = rgb2gray(distractor_resized)
            distractor_resized = 255 * distractor_resized
            self.distractor_img = torch.from_numpy(distractor_resized)

            if fixed_didactic:
                # Find all locations which don't overlap with original image
                nonoverlapping_coords = get_nonoverlapping_coords(dim, padding)
                self.poss_distractor_coords = {i:nonoverlapping_coords.copy() for i in range(10)}
                for i in range(10):
                    # Remove any coordinates which overlap with the didactic signal
                    did_row, did_col = self.didactic_coords[i]
                    self.poss_distractor_coords[i] = remove_overlapping(did_row, did_col,self.poss_distractor_coords[i],
                                                                padding, dim)
            else:
                self.poss_distractor_coords = get_nonoverlapping_coords(dim, padding)
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if self.false_didactic:
            ...
            # Add distractor to the didactic image
        ...
        # Add distractor signal to the image
        if self.distractor:
            ...
            if self.fixed_didactic:
                row, col = self.poss_distractor_coords[target][np.random.choice(len(self.poss_distractor_coords[target]))]

            else:
                # Remove any coordinates which overlap with the didactic signal
                poss_distractor_coords = self.poss_distractor_coords.copy()
                poss_distractor_coords = remove_overlapping(did_row, did_col, poss_distractor_coords,
                                   self.padding, self.dim)
                # Randomly select distractor coordinate
                row, col = poss_distractor_coords[np.random.choice(len(poss_distractor_coords))]

            img[row:row+self.padding,col:col+self.padding] = self.distractor_img
...
if __name__ == "__main__":
    ...
    train_data = MNIST(padding=12, root='/home/eric/Datasets', train=True, download=False, didactic=False,
                       distractor=False, unique_didactic=False, false_didactic=False, fixed_didactic=False,
                       random_labels=False)
    ...
    train_data = MNIST(padding=12, root='/home/eric/Datasets', train=True, download=False, didactic=True,
                       distractor=False, unique_didactic=False, false_didactic=False, fixed_didactic=True,
                       random_labels=False)
    plot_images('./sample_plots/experiment_1.png', train_data)

    # Experiment 2
    train_data = MNIST(padding=12, root='/home/eric/Datasets', train=True, download=False, didactic=True,
                       distractor=True, unique_didactic=False, false_didactic=False, fixed_didactic=True,
                       random_labels=False)
    plot_images('./sample_plots/experiment_2.png', train_data)

    # Experiment 3
    train_data = MNIST(padding=12, root='/home/eric/Datasets', train=True, download=False, didactic=True,
                       distractor=False, unique_didactic=True, false_didactic=False, fixed_didactic=True,
                       random_labels=False)
    plot_images('./sample_plots/experiment_3.png', train_data)

    # Experiment 4
    train_data = MNIST(padding=12, root='/home/eric/Datasets', train=True, download=False, didactic=True,
                       distractor=True, unique_didactic=True, false_didactic=False, fixed_didactic=True,
                       random_labels=False)
    plot_images('./sample_plots/experiment_4.png', train_data)

    # Experiment 5
    train_data = MNIST(padding=12, root='/home/eric/Datasets', train=True, download=False, didactic=True,
                       distractor=False, unique_didactic=True, false_didactic=False, fixed_didactic=False,
                       random_labels=False)
    plot_images('./sample_plots/experiment_5.png', train_data)

    # Experiment 6
    train_data = MNIST(padding=12, root='/home/eric/Datasets', train=True, download=False, didactic=True,
                       distractor=True, unique_didactic=True, false_didactic=False, fixed_didactic=False,
                       random_labels=False)
    plot_images('./sample_plots/experiment_6.png', train_data)

    # Experiment 7
    train_data = MNIST(padding=12, root='/home/eric/Datasets', train=True, download=False, didactic=True,
                       distractor=False, unique_didactic=False, false_didactic=True, fixed_didactic=True,
                       random_labels=False)
    plot_images('./sample_plots/experiment_7.png', train_data)

    # Experiment 8
    train_data = MNIST(padding=12, root='/home/eric/Datasets', train=True, download=False, didactic=True,
                       distractor=False, unique_didactic=True, false_didactic=True, fixed_didactic=True,
                       random_labels=False)
    plot_images('./sample_plots/experiment_8.png', train_data)

    # Experiment 9
    train_data = MNIST(padding=12, root='/home/eric/Datasets', train=True, download=False, didactic=True,
                       distractor=False, unique_didactic=True, false_didactic=True, fixed_didactic=False,
                       random_labels=False)
